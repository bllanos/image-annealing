...
fn main() {
    ...
    println!(
        ...
        config.count_swap.as_ref().unwrap().0.display()
    );
    println!(
    ...
    println!(
        ...
        config.create_permutation.as_ref().unwrap().0.display()
    );
    println!(
        "cargo:rustc-env=PERMUTE_SHADER={}",
        config.permute.as_ref().unwrap().0.display()
    );
    println!(
        "cargo:rustc-env=SWAP_SHADER={}",
        config.swap.as_ref().unwrap().0.display()
    );
