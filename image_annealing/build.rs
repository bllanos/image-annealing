...
fn main() {
    ...
    println!(
        ...
        config.count_swap.as_ref().unwrap()
    );
    println!(
        "cargo:rustc-env=CREATE_DISPLACEMENT_GOAL_DEFAULT_SHADER={}",
        config.create_displacement_goal_default.as_ref().unwrap()
    );
    println!(
        "cargo:rustc-env=CREATE_PERMUTATION_SHADER={}",
        config.create_permutation.as_ref().unwrap()
    );
    println!(
        "cargo:rustc-env=PERMUTE_SHADER={}",
        config.permute.as_ref().unwrap()
    );
    println!(
        "cargo:rustc-env=SWAP_SHADER={}",
        config.swap.as_ref().unwrap()
    );
