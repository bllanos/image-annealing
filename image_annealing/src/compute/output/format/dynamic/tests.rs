...
mod lossless_image {
    ...
    fn valid_rgba8_buffer() -> image::RgbaImage {
        image::RgbaImage::from_pixel(2, 1, image::Rgba([0, 0, 0, 0]))
    }

    fn valid_rgba16_buffer() -> Rgba16ImageBuffer {
        Rgba16ImageBuffer::from_pixel(2, 1, image::Rgba([0, 0, 0, 0]))
    }

    fn valid_rgba8() -> LosslessImage {
        LosslessImage::Rgba8(Rgba8Image::new(valid_rgba8_buffer()).unwrap())
    }

    fn valid_rgba8x2() -> LosslessImage {
        LosslessImage::Rgba8x2(
            Rgba8x2Image::new(valid_rgba8_buffer(), valid_rgba8_buffer()).unwrap(),
        )
    }
    ...
    fn valid_rgba16() -> LosslessImage {
        LosslessImage::Rgba16(Rgba16Image::new(valid_rgba16_buffer()).unwrap())
    }

    fn valid_rgba16x2() -> LosslessImage {
        LosslessImage::Rgba16x2(
            Rgba16x2Image::new(valid_rgba16_buffer(), valid_rgba16_buffer()).unwrap(),
        )
    }

    fn valid_rgba16_rgba8() -> LosslessImage {
        LosslessImage::Rgba16Rgba8(
            Rgba16Rgba8Image::new(valid_rgba16_buffer(), valid_rgba8_buffer()).unwrap(),
        )
    }

    fn valid_rgba16_rgba8x2() -> LosslessImage {
        LosslessImage::Rgba16Rgba8x2(
    ...
    mod io {
        ...
        mod success {
            ...
            use std::error::Error;
            use std::path::PathBuf;
            use test_util::image::{DimensionsAndRgba16Buffer, DimensionsAndRgba8Buffer};
            ...
            fn rgba8() -> Result<(), Box<dyn Error>> {
                let DimensionsAndRgba8Buffer { image, .. } =
                    test_util::image::linear_indices_with_bias_to_colors(0);
            ...
            fn rgba8x2() -> Result<(), Box<dyn Error>> {
                ...
                let DimensionsAndRgba8Buffer { image: image2, .. } =
                    test_util::image::linear_indices_with_bias_to_colors(
                        *image1.last().unwrap() + 1,
                    );
                let output_image =
            ...
            fn rgba16_rgba8() -> Result<(), Box<dyn Error>> {
                ...
                let output_image = LosslessImage::Rgba16Rgba8(Rgba16Rgba8Image::new(
                    image1.clone(),
                    image2.clone(),
                )?);
                ...
                let input_image =
                    LosslessImage::load(ImageFormat::Rgba16Rgba8, &full_output_paths)?;
            ...
            fn rgba16_rgba8x2() -> Result<(), Box<dyn Error>> {
                ...
                let input_image =
                    LosslessImage::load(ImageFormat::Rgba16Rgba8x2, &full_output_paths)?;
        ...
        mod first_image_missing {
            use super::super::super::super::{ImageFormat, LosslessImage};

            #[test]
            fn rgba8() {
                test_util::assert_error_contains(
                    LosslessImage::load(ImageFormat::Rgba8, &[super::missing_image_path()]),
            ...
            fn rgba16() {
                test_util::assert_error_contains(
                    LosslessImage::load(ImageFormat::Rgba16, &[super::missing_image_path()]),
                    super::missing_error_message(),
                );
            }
        ...
        mod first_second_mismatch {
            ...
            fn rgba16_rgba8() {
                test_util::assert_error_contains(
                    LosslessImage::load(
                        ImageFormat::Rgba16Rgba8,
                        &[super::existing_rgba16_path(), super::large_rgba8_path()],
                    ),
                    super::mismatch_error_message(),
                );
            }
        ...
        mod first_image_save_error {
            ...
            fn rgba8() {
                test_util::assert_error_contains(
                    super::super::valid_rgba8()
                        .save_add_extension(&[super::missing_directory_path()]),
                    super::missing_error_message(),
                );
            }
