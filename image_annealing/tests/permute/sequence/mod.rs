...
use image_annealing::compute::{
    self, Config, CreateDisplacementGoalInput, OutputStatus, PermuteInput, PermuteParameters,
    SwapInput,
};
...
fn ignore_identity_displacement_goal() -> Result<(), Box<dyn Error>> {
    ...
    let original_image = test_util::image::coordinates_to_colors(&dimensions);
    let permuted_image = test_util::permutation::non_identity_forward_permute(&original_image);
    let original_lossless_image = LosslessImage::Rgba16(Rgba16Image::new(original_image)?);
...
fn reuse_swap_permutation() -> Result<(), Box<dyn Error>> {
    ...
    let original_image = test_util::image::coordinates_to_colors(&dimensions);
    let permuted_image =
        test_util::permutation::non_identity_horizontal_swap_forward_permute(&original_image);
    let original_lossless_image = LosslessImage::Rgba16(Rgba16Image::new(original_image)?);
...
fn reuse_image() -> Result<(), Box<dyn Error>> {
    ...
    let original_image = test_util::image::coordinates_to_colors(&dimensions);
    let mut permuted_image = test_util::permutation::identity_permute(&original_image);
    ...
    permuted_image = test_util::permutation::non_identity_forward_permute(&original_image);
...
fn reuse_permutation() -> Result<(), Box<dyn Error>> {
    ...
    let expected_permutation = permutation.clone();
    let mut original_image = test_util::image::coordinates_to_colors(&dimensions);
    ...
    original_image = test_util::image::coordinates_to_zero_alpha_colors(&dimensions);
    permuted_image =
        test_util::permutation::bit_interpretation_cases_forward_permute(&original_image);
